...
const [showMobileMenu, setShowMobileMenu] = useState(false)
  ...
  useEffect(() => {
    ...
    return (
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className='absolute top-0 left-0 w-full z-10'>
      <div className='container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent'>


            <img src={assets.favicon} alt="Afri Kan-excel" className="w-10 h-10"/>
        ...
        <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
          ...
          <a onClick={() => setShowMobileMenu(false)} href="#Donate" className='px-4 py-2 rounded-full inline-block bg-blue-600 text-white'>Donate</a>
        </ul>
        </div> 
      </motion.div>
    )
